...
class RushGUI(object):
    ...
    def rush_setting(self):
        rush_setting_frame = Frame(self.root, borderwidth=3, relief="sunken")
        sku_ids_label = Label(rush_setting_frame, text="sku_ids:")
        sku_ids_entry = Entry(rush_setting_frame, textvariable=self.sku_ids)

        sku_ids_button = Button(rush_setting_frame, text='确定', command=self.reset_sku_ids)
        sku_ids_label.pack(side=LEFT)
        sku_ids_entry.pack(side=LEFT)
        sku_ids_button.pack(side=LEFT)
        return rush_setting_frame
...
class SkuFrame(Frame):
    def __init__(self, master=None, sku_index=-1, cnf={}, **kw):
        ...
        self.sku_id = StringVar(master=self, name="sku_ids")
        self.quality = StringVar(master=self, name="quality")
...
if __name__ == '__main__':
    root = Tk()
    root.geometry("%dx%d" % (800, 600))  # 窗体尺寸
    plan_control = PlanControl(root, borderwidth=3, relief="sunken")
    plan_control.pack()
    # jd_user.login()
    root.mainloop()
